import { Link, Outlet, useNavigate } from 'react-router-dom';
import { useAuth } from '../../hooks/useAuth';
import Button from '../ui/Button';

export default function AdminLayout() {
  const { logout } = useAuth();
  const navigate = useNavigate();

  const handleLogout = () => {
    logout();
    navigate('/login');
  };

  const menuItems = [
    { label: 'Sinh viên', href: '/admin/students' },
    { label: 'Phòng', href: '/admin/rooms' },
    { label: 'Thanh toán', href: '/admin/payments' },
    { label: 'Đăng ký phòng', href: '/admin/registrations' },
    { label: 'Tiện ích', href: '/admin/utilities' },
    { label: 'Khiếu nại', href: '/admin/complaints' },
  ];

  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className="w-64 bg-white shadow-lg">
        <div className="p-6 border-b border-red-200 bg-gradient-to-r from-red-50 to-red-100">
          <h1 className="text-2xl font-bold text-red-600">Admin</h1>
          <p className="text-sm text-gray-600 mt-1">Quản lý ký túc xá</p>
        </div>
        <nav className="mt-6 space-y-2 px-4">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              to={item.href}
              className="block px-4 py-2 rounded-lg text-gray-700 hover:bg-red-50 hover:text-red-600 transition cursor-pointer"
            >
              {item.label}
            </Link>
          ))}
        </nav>
        <div className="absolute bottom-6 left-4 right-4">
          <Button
            onClick={handleLogout}
            className="w-full bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg cursor-pointer transition"
          >
            Đăng xuất
          </Button>
        </div>
      </div>

      {/* Main Content */}
      <div className="flex-1 overflow-auto">
        <div className="p-8">
          <Outlet />
        </div>
      </div>
    </div>
  );
}
